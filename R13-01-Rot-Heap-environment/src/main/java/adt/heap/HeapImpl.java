	...
	private void heapify(int position) {
		if (position >= 0){
			if (!isEmpty()){
				...
				if (left <= this.index && this.comparator.compare(this.heap[largest], this.heap[left]) < 0){
					largest = left;
				}
				if (right <= this.index && this.comparator.compare(this.heap[largest], this.heap[right]) < 0){
					largest = right;
				}
				if (position != largest){
